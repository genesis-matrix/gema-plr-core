...
salt-highstate-test-on-%:
	## $@ ##
	-@vagrant ssh salt-master-d1 -c 'sudo salt -C "$(@:salt-highstate-test-on-%=%)" state.test --force-color '
...
salt-highstate-show-as-json-on-%:
	## $@ ##
	@vagrant ssh salt-master-d1 -c 'sudo salt -C "$(@:salt-highstate-show-as-json-on-%=%)" state.show_highstate --out=json '
